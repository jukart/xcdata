settings = {
    "sisteron": {
        "links": {
            "data/airspace1.txt": "airspaces/france.txt",
            "data/airspace2.txt": None,
            "data/airfields.txt": None,
            "data/waypoint1.cup": "waypoints/France.cup",
            "data/waypoint2.cup": None,  # "waypoints/ofm.cup",
            "data/watched.cup": None,
            "data/map.xcm": "maps/ALPS_HighRes.xcm",
            "data/checklist.txt": "checklists/sisteron.txt",
            "data/events.xci": "xcsoar/events.xci",
            "data/plane.xcp": "planes/D-5461-18m.xcp",
            "data/tasks": "tasks",
        },
        "copy": {
            "default.prf": "profiles/sisteron.prf",
            "Default.tsk": None,
        },
    },
    "_loih": {
        "links": {
            "data/airspace1.txt": "airspaces/lovv.desktop.strepla.openair.txt",
            "data/airspace2.txt": None,
            "data/airfields.txt": None,
            "data/waypoint1.cup": "waypoints/Austria.cup",
            "data/waypoint2.cup": "waypoints/ofm.cup",
            "data/watched.cup": None,
            "data/map.xcm": "maps/ALPS_HighRes.xcm",
            "data/checklist.txt": "checklists/loih.txt",
            "data/events.xci": "xcsoar/events.xci",
            "data/plane.xcp": "planes/D-5461-18m.xcp",
            "data/tasks": "tasks",
        },
        "copy": {
            "default.prf": "profiles/speed.prf",
            "Default.tsk": None,
        },
    },
    "_loih_task": {
        "links": {
            "data/airspace1.txt": "airspaces/LOIH_2017v1_LR_openair.txt",
            "data/airspace2.txt": None,
            "data/airfields.txt": None,
            "data/waypoint1.cup": "waypoints/Austria.cup",
            "data/waypoint2.cup": "waypoints/LOIH_2017v1_WP.cup",
            "data/watched.cup": None,
            "data/map.xcm": "maps/ALPS_HighRes.xcm",
            "data/checklist.txt": "checklists/loih.txt",
            "data/events.xci": "xcsoar/events.xci",
            "data/plane.xcp": "planes/D-5461-18m.xcp",
            "data/tasks": "tasks",
        },
        "copy": {
            "default.prf": "profiles/minimal-speed.prf",
            "Default.tsk": None,
        },
    },
    #"stm-2017-speed": {
    #    "links": {
    #        "data/airspace1.txt": "airspaces/ssm2017-2_openair.txt",
    #        "data/airspace2.txt": None,
    #        "data/airfields.txt": None,
    #        "data/waypoint1.cup": "waypoints/WP-ssm-2017-6.cup",
    #        "data/waypoint2.cup": "waypoints/Austria.cup",
    #        "data/watched.cup": None,
    #        "data/map.xcm": "maps/ALPS_HighRes.xcm",
    #        "data/checklist.txt": "checklists/loih.txt",
    #        "data/events.xci": "xcsoar/events.xci",
    #        "data/plane.xcp": "planes/D-5461-18m.xcp",
    #        "data/tasks": "tasks",
    #    },
    #    "copy": {
    #        "default.prf": "profiles/minimal-speed.prf",
    #        "Default.tsk": "tasks/stm-2017-day5.tsk",
    #    },
    #},
    #"stm-2017-aat": {
    #    "links": {
    #        "data/airspace1.txt": "airspaces/ssm2017-2_openair.txt",
    #        "data/airspace2.txt": None,
    #        "data/airfields.txt": None,
    #        "data/waypoint1.cup": "waypoints/WP-ssm-2017-6.cup",
    #        "data/waypoint2.cup": "waypoints/Austria.cup",
    #        "data/watched.cup": None,
    #        "data/map.xcm": "maps/ALPS_HighRes.xcm",
    #        "data/checklist.txt": "checklists/loih.txt",
    #        "data/events.xci": "xcsoar/events.xci",
    #        "data/plane.xcp": "planes/D-5461-18m.xcp",
    #        "data/tasks": "tasks",
    #    },
    #    "copy": {
    #        "default.prf": "profiles/minimal-aat.prf",
    #        "Default.tsk": "tasks/stm-2017-day4.tsk",
    #    },
    #},
    #"aac-2017-speed": {
    #    "links": {
    #        "data/airspace1.txt": "airspaces/AAC2016-2_openair.txt",
    #        "data/airspace2.txt": None,
    #        "data/airfields.txt": None,
    #        "data/waypoint1.cup": "waypoints/AAC2016.cup",
    #        "data/waypoint2.cup": "waypoints/ofm.cup",
    #        "data/watched.cup": None,
    #        "data/map.xcm": "maps/ALPS_HighRes.xcm",
    #        "data/checklist.txt": "checklists/loih.txt",
    #        "data/events.xci": "xcsoar/events_aac.xci",
    #        "data/plane.xcp": "planes/D-5461-18m.xcp",
    #        "data/tasks": "tasks",
    #    },
    #    "copy": {
    #        "default.prf": "profiles/speed.prf",
    #        "Default.tsk": "tasks/aac-2017-offen-day5.tsk",
    #    },
    #},
    #"aac-2017-aat": {
    #    "links": {
    #        "data/airspace1.txt": "airspaces/AAC2016-2_openair.txt",
    #        "data/airspace2.txt": None,
    #        "data/airfields.txt": None,
    #        "data/waypoint1.cup": "waypoints/AAC2016.cup",
    #        "data/waypoint2.cup": "waypoints/ofm.cup",
    #        "data/watched.cup": None,
    #        "data/map.xcm": "maps/ALPS_HighRes.xcm",
    #        "data/checklist.txt": "checklists/loih.txt",
    #        "data/events.xci": "xcsoar/events_aac.xci",
    #        "data/plane.xcp": "planes/D-5461-18m.xcp",
    #        "data/tasks": "tasks",
    #    },
    #    "copy": {
    #        "default.prf": "profiles/aat.prf",
    #        "Default.tsk": "tasks/aac-2017-offen-day6.tsk",
    #    },
    #},
}
